...
/**
 * The self-describing, suite-bound invitation (the migration brief §4): the A3 link carries the
 * bundle's suite out of band, the bundle's signature verifies under its own suite, and the
 * joiner's downgrade guard is link-vs-bundle suite equality.
 */
class InviteBundleTest {
    ...
    private fun bundle() = InviteBundle.build(
        provider,
    ...
    // ── The link's suite hint ─────────────────────────────────────────────────────────────────

    @Test
    fun link_carriesTheInviterIdentitysSuite() {
        val link = InviteLink.create(provider, secret, inviterKeys.identity)
    ...
    @Test
    fun linkSuiteId_mustBeCanonicalDecimal() {
        val parts = InviteLink.create(provider, secret, inviterKeys.identity).fragment().split('.')
    ...
    @Test
    fun bundle_verifiesAndRoundTripsByteExactly() {
        val built = bundle()
        ...
        val decoded = InviteBundle.deserialise(bytes, resolver)
    ...
    @Test
    fun bundleSignature_coversTheRecipientRelevantMaterial() {
        val built = bundle()
    ...
    fun joinerDowngradeGuard_linkAndBundleSuitesMustAgree() {
        ...
        // downgrades the bundle's suite (here: an otherwise-valid bundle from a Suite 1
        // identity) is caught by the equality check every joiner performs.
        val link = InviteLink.create(provider, secret, inviterKeys.identity)
        val suite1Inviter = DeviceKeys.generate(provider, Suite1)
        val downgraded = InviteBundle.build(
